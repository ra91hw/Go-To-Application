...
var ZYFILE = {
    ...
    funUploadFiles : function() {
        var self = this;
        self.funBatchUploadFile(this.uploadFile);
    },
    ...
    funBatchUploadFile : function(files) {
        var self = this;
        if(files.length > 9){
            alert("Upload up to 9 photos！");
            return;
        }
        var formdata = new FormData();
        ...
        // var customData = JSON.parse(ZYFILE.customData);
        // customData.post = $('#id').val();
        formdata.append("albumId", $('#id').val());
        var xhr = new XMLHttpRequest();
        xhr.upload.addEventListener("progress", function(e) {
            $.each(files, function(k, v) {
        ...
        xhr.addEventListener("load", function(e) {
            self.onSuccess(files, xhr.responseText);
            $.each(files, function(k, v) {
            ...
            .each(function() {
                ...
                this.createHtml = function() {
                    ...
                    if (para.dragDrop) {
                        ...
                        html += '				<div class="btns">';
                        html += '					<div class="webuploader_pick"><i class="fa fa-upload">Image</i></div>';
                        html += '					<div class="upload_btn"><i class="fa fa-send">Upload</i></div>';
                        html += "				</div>";
                        html += "			</div>";
                        ...
                        html += '					<a style="height:'
                            ...
                            + ";width:"
                            + para.itemWidth
                            + ';" title="Click to add file" id="rapidAddImg" class="add_imgBox" href="javascript:void(0)">';
                ...
                this.funSetStatusInfo = function(files) {
                    ...
                    $("#status_info").html(
                        "Select" + num + "files，a total of" + size + ".")
                };
                        ...
                        if (newStr.split(".")[0] != null) {
                            ...
                            if (jQuery.inArray(type, para.fileType) > -1) {
                                if (file.size >= para.fileSize) {
                                    alert('Your"' + file.name + '"file is too big')
                                } else {
                                    arrFiles.push(file)
                                }
                            } else {
                ...
                this.funDisposePreviewHtml = function(file, e) {
                    ...
                    if (para.tailor) {
                        editHtml = '<span class="file_edit" data-index="'
                            + file.index + '" title="Edit"></span>'
                    }
                    if (para.del) {
                        delHtml = '<span class="file_del" data-index="'
                            + file.index + '" title="Delete"></span>'
                    }
                    var newStr = file.name.split("").reverse().join("");
                    ...
                    if (file.type.indexOf("image") == 0) {
                        ...
                        html += '	<p id="uploadFailure_' + file.index
                            + '" class="file_failure">Upload failed, please try again</p>';
                        html += '	<p id="uploadTailor_'
                            + file.index
                            + '" class="file_tailor" tailor="false">Cropping is complete</p>';
                ...
                this.createCorePlug = function() {
                    ...
                    var params = {
                        ...
                        onFailure : function(file, response) {
                            ...
                            $("#uploadInf").append(
                                "<p>The file" + file.name + "upload failed！</p>")
                        },
                        onComplete : function(response) {
                ...
                this.addEvent = function() {
                    ...
                    $(".upload_btn").bind("click", function(e) {
                        if (ZYFILE.funReturnNeedFiles().length > 0) {
                            ...
                            layer.msg("Please select the picture and click to upload！", {icon: 5, time: 2000, shift: 6, shade: [0.5, '#393D49']});
                        }
                    });
