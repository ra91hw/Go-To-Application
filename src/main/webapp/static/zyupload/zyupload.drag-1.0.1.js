...
//					edit             : true,
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
		            ...
		            html += '					<div class="upload_btn">The file start uploading now</div>';
		            html += '				</div>';
		            html += '			</div>';
		            ...
		            html += '				<div class="btns">';
		            html += '					<input id="fileImage" type="file" size="30" name="fileselect[]" '+multiple+'>';
		            html += '					<div class="webuploader_pick">Select the file</div>';
		            html += '					<div class="upload_btn">The file start uploading now</div>';
		            html += '				</div>';
		            html += '			</div>';
					...
					if(newStr.split(".")[0] != null){
						...
						if(jQuery.inArray(type, para.fileType) > -1){
							if (file.size >= para.fileSize) {
								alert('Your"'+ file.name +'" size is too large');
							} else {
								arrFiles.push(file);	
							}
						}else{
							alert('Your"'+ file.name +'"does not match the upload type');
						}
					}else{
						alert('Your"'+ file.name +'"has no type and cannot be recognized');
					}
				}
				return arrFiles;
			};
			...
			this.funDisposePreviewHtml = function(file, e){
				...
				if(para.tailor){
					editHtml = '<span class="file_edit" data-index="'+file.index+'" title="Edit"></span>';
				}
				if(para.del){
