...
//					edit             : true,  						      // 是否可以编辑文件（裁剪）
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
		            ...
		            html += '					<div class="upload_btn">The file start uploading</div>';
		            html += '				</div>';
		            html += '			</div>';
		            ...
		            html += '				<div class="btns">';
		            html += '					<input id="fileImage" type="file" size="30" name="fileselect[]" '+multiple+'>';
		            html += '					<div class="webuploader_pick">Choose the file</div>';
		            html += '					<div class="upload_btn">The file start uploading</div>';
		            html += '				</div>';
		            html += '			</div>';
					...
					if(newStr.split(".")[0] != null){
						...
						if(jQuery.inArray(type, para.fileType) > -1){
							...
							if (file.size >= para.fileSize) {
								alert('Your"'+ file.name +'"file size is too big');
							} else {
								// 在这里需要判断当前所有文件中
								arrFiles.push(file);	
							}
						}else{
							alert('Your"'+ file.name +'"Upload type does not match');
						}
					}else{
