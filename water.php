	...
	<body>
		...
		<section id="content">
			<div id="pictures" class="pictures">
				<div class="content">
						<h1>Between the sea and oceans, rivers and lakes, photo opportunities near bodies of water are plentiful</h1>
						<h2>Recently uploaded water photos:</h2>
						<?php 
							$query = "SELECT CONCAT(path, newFileName, '.', ext) AS imgname, t_files.id AS imgId, t_user.id AS userId, t_user.username AS username, t_user.avatar AS avatarExt FROM t_files JOIN t_user ON t_files.userId=t_user.id WHERE category='water' ORDER BY t_files.uploadtime DESC LIMIT 20 OFFSET " . ($page*20); //Gets 20 file names including extension
							
							//Count the results of a different query (since the previous one is limited)
							$photoCount = mysqli_num_rows(mysqli_query($connection, "SELECT CONCAT(newFileName, '.', ext) AS imgname, t_user.id AS userId, t_user.username AS username FROM t_files JOIN t_user ON t_files.userId=t_user.id WHERE category='water'"));
