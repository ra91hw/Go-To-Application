	...
	<body>
		...
		<section id="content">
			<div id="pictures" class="pictures">
				<div class="content">
						<!--The below statement is not factually incorrect but it is very odd. I'd reword it if I could think of anything.!-->
						<h1>Buildings, fountains, and statues make for some very nice photographs thanks to the work of architects and artists everywhere.</h1>
						<h2>Recently uploaded structures photos:</h2>
						<?php 
							$query = "SELECT CONCAT(path, newFileName, '.', ext) AS imgname, t_files.id AS imgId, t_user.id AS userId, t_user.username AS username, t_user.avatar AS avatarExt FROM t_files JOIN t_user ON t_files.userId=t_user.id WHERE category='structures' ORDER BY t_files.uploadtime DESC LIMIT 20 OFFSET " . ($page*20); //Gets 20 file names including extension
							
							//Count the results of a different query (since the previous one is limited)
							$photoCount = mysqli_num_rows(mysqli_query($connection, "SELECT CONCAT(newFileName, '.', ext) AS imgname, t_user.id AS userId, t_user.username AS username FROM t_files JOIN t_user ON t_files.userId=t_user.id WHERE category='structures'"));
